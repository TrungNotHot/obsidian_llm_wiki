...
1. **Automated Graph & Vault Integrity Verification**:
   - Run the bundled standalone health check script via terminal (`rtk`):
     ```bash
     rtk python3 ~/.gemini/config/plugins/obsidian-llm-wiki/skills/kb-health/scripts/check_health.py docs
     ```
...
2. **Schema & Tag Taxonomy Validation**:
   - Verify note frontmatter conforms to `docs/SCHEMA.md`.
...
3. **Code vs Doc Drift Auditing**:
   - Compare active components, modules, services, or data contracts in the codebase against documentation in `docs/wiki/`.
   - Verify if newly added/modified modules or interfaces have matching entity pages in `docs/wiki/entities/`.

4. **Generate or Update `docs/wiki/TODO.md`**:
